...
class TaskEmothaw:
    ...
    def __init__(self, subject_id: int, task_number: int, strokes_list: list[Stroke], all_coords: list[tuple[int, int, int, int, int, int, int]], d_status=0, s_status=0, a_status=0, rep_type: RepresentationType = None, cache_base_dir = cache_dir):
    ...
    def _rep_simple_stroke(self):
        width_range = self.max_vals['x_surface'] - self.min_vals['x_surface']
        height_range = self.max_vals['y_surface'] - self.min_vals['y_surface']
        scale = 0.3
        final_w = int(width_range * scale)
        final_h = int(height_range * scale)
        ...
        for letters_set in self.letters_sets_list:
            for stroke in letters_set.strokes_list:
                ...
                normalized_x = [
                    int((x - self.min_vals['x_surface']) * scale)
                    for x in stroke_x_list
                ]

                normalized_y = [
                    int((y - self.min_vals['y_surface']) * scale)
